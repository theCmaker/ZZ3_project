...
InsertMove::InsertMove(Problem & p, const Mobile & m_in, int m_prev, const Interceptor & i) : Move(p), _mobile_in(m_in), _mobile_prev(m_prev), _interceptor(i) {}
...
bool InsertMove::scan(const Solution & solution)
{
	...
	if(_mobile_prev >= 0)
	{
		// compute time of interception of mobile_prev (mobile before the insertion)
	...
	// sum with the interception time of the mobile inserted 
	...
	// check if there is no problem in the route after the insertion
	// and compute the interception time for each following mobile
	...
	// compare time of interception between the update route and the worse route (in time)
...
void InsertMove::commit(Solution &)
{
