...
void ParetoFrontSolver<POINT>::setPts(FPointv &v)
{
	...
	for (unsigned i =  0; i < nb_pts_; ++i) {
		if (pts_[i].getX() < x_min_) {
			x_min_ = pts_[i].getX();
		}
		if (pts_[i].getX() > x_max_) {
			x_max_ = pts_[i].getX();
		}
		if (pts_[i].getY() < y_min_) {
			y_min_ = pts_[i].getY();
		}
		if (pts_[i].getY() > y_max_) {
			y_max_ = pts_[i].getY();
		}
	}
}
...
void ParetoFrontSolver<POINT>::compute_frontiers() {
	...
	for (typename FPointv::iterator i = pts_.begin(); i != pts_.end(); ++i) { //For each point
		...
		if (x_itr != pts_map_.end()) { //Submap exists
			...
			pts_map_.insert(std::pair<float,FPointPtrMap >(i->getX(),ymap)); //Insert the submap
		}
	}
 ...
 */
template <typename POINT>
void ParetoFrontSolver<POINT>::compute_inclusive_frontiers() {
	//Build Map
	typename FPointPtrMMap::iterator x_itr;
	for (typename FPointv::iterator i = pts_.begin(); i != pts_.end(); ++i) { //For each point
		x_itr = pts_map_.find(i->getX()); //Look for the abscissa (if submap exists)
	...
	while (!pts_map_copy.empty()) { //While there are remaining points
		...
		POINT *current = NULL;
		pFrontiers_.push_back(ParetoFront<POINT>()); //New empty front
		auto range = pts_map_copy.begin()->second.equal_range(pts_map_copy.begin()->second.begin()->first);

		for (auto i = range.first; i != range.second; ++i)
		{
			pFrontiers_.back().pts().push_back(*i->second); //Add first point to the front
		}
		current = & pFrontiers_.back().pts().front();  //Set current point
		pts_map_copy.begin()->second.erase(current->getY()); //Remove the point in the map
		...
		if (!pts_map_copy.empty()) {        //If there are some remaining points in the map
			...
			while (x_itr != pts_map_copy.end()) { //Some points have not been tested for insertion
				if (x_itr->second.begin()->second->dominates(*current)) { //If the last inserted point is dominated
					auto range = x_itr->second.equal_range(x_itr->second.begin()->first);

					for (auto i = range.first; i != range.second; ++i) {
						pFrontiers_.back().pts().push_back(*i->second); //Insertion
					}
					current = x_itr->second.begin()->second;    //Update the last inserted point
					x_itr->second.erase(current->getY());       //Remove the point in the map
